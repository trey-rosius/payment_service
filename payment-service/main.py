...
def initiate_payment(payment: PaymentModel):
    instance_id = wf_client.schedule_new_workflow(
        workflow=payment_workflow, input=payment.model_dump()
    )
...
def payment_workflow(ctx: wf.DaprWorkflowContext, wf_input: any):
    try:
        ...
        payment_intent_response = yield ctx.call_activity(
            create_payment_intent, input=wf_input
        )
        if payment_intent_response["status"] == "success":
            ...
            if winner == timeout_event:
                yield ctx.call_activity(
                    ...
                    input={
                        "payment_intent_id": payment_intent_response[
                            "payment_intent_id"
                        ],
...
def create_payment_intent(ctx, activity_input: any):
    with DaprClient() as d:
        ...
        payment_intent = client.payment_intents.create(
            params={"amount": activity_input["amount"], "currency": "usd"}
        )
        ...
        return {"payment_intent_id": payment_intent["id"], "status": "success"}
